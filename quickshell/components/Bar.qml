...
PanelWindow {
    ...
    margins { top: 4; left: 0; right: 0 }
    implicitHeight: 32
    color: "transparent"

    property color notchColor: Qt.rgba(0, 0, 0, 0.85)
    property color notchHoverColor: Qt.rgba(0, 0, 0, 0.92)
    property int notchRadius: 12
    property int notchHeight: 32

    property int activeWsId: 1
    property int targetWsId: 1
    property string activeWsName: ""
    property string targetWsName: ""
    ...
    property var sortedWorkspaces: {
        ...
        list.sort(function(a, b) {
            var aIsNamed = a.name && a.name !== "" && a.id < 0
            var bIsNamed = b.name && b.name !== "" && b.id < 0
            if (!aIsNamed && !bIsNamed) return a.id - b.id
    ...
    Connections {
        ...
        function onRawEvent(event) {
            if (event.name === "workspace") {
                ...
                var wsId = parseInt(data)
                if (!isNaN(wsId)) {
                    bar.targetWsId = wsId
                    bar.targetWsName = ""
                } else {
                    bar.targetWsName = data
                    bar.targetWsId = -999
                }
                ...
                if (parts.length >= 2) {
                    var data2 = parts[1].trim()
                    var wsId2 = parseInt(data2)
                    if (!isNaN(wsId2)) {
                        bar.targetWsId = wsId2
                        bar.targetWsName = ""
                    } else {
                        bar.targetWsName = data2
                        bar.targetWsId = -999
                    }
                    wsTransition.restart()
                }
    ...
    SequentialAnimation {
        ...
        ScriptAction {
            script: { bar.activeWsId = bar.targetWsId; bar.activeWsName = bar.targetWsName }
        }
        ParallelAnimation {
            PropertyAnimation {
                target: wsHighlight
            ...
            Notch {
                ...
                Item {
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Item {
                                    ...
                                    property bool isActive: bar.activeWsName !== "" ? (modelData.name === bar.activeWsName) : (bar.activeWsId === modelData.id)
                                    property bool isHovered: wsMA.containsMouse

                                    visible: modelData.id > 0 || (modelData.name && modelData.name !== "")
                                    ...
                                    MouseArea {
                                        ...
                                        hoverEnabled: true
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: Hyprland.dispatch("workspace " + (modelData.name && modelData.name !== "" && modelData.id < 0 ? "name:" + modelData.name : modelData.id))
                                    }
                                }
                            }
                        }
                        ...
                        Connections {
                            target: bar
                            function onActiveWsIdChanged() { updateActiveHighlight() }
                            function onActiveWsNameChanged() { updateActiveHighlight() }
